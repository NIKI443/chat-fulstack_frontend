	...
	'fixed z-50 gap-4 bg-background p-6 transition ease-in-out data-[state=closed]:duration-300 data-[state=open]:duration-500 data-[state=open]:animate-in data-[state=closed]:animate-out',
	{
		variants: {
			side: {
				...
				custom:
					'inset-y-0 inset-x-0 right-0 h-full data-[state=closed]:slide-out-to-right data-[state=open]:slide-in-from-right',
			},
			sideOverlay: {
				custom:
...
>(
	(
		{ side = 'custom', sideOverlay = 'custom', className, children, ...props },
		ref
	) => (
		<SheetPortal>
			<SheetOverlay className={cn(sheetVariants({ sideOverlay }))} />
